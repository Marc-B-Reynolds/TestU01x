...
static uint32_t isQuestionablePvalue(double pvalue)
{
  ...
  // questionable p-value
  if(pvalue < ALPHA || pvalue > 1.0-ALPHA) {
    return 1;
  }
 ...
 */
{
  ...
  for(i = 0; i < Rep[j2]; ++i) {
    ...
    for(uint32_t cnt=0; cnt<RERUN_LIMIT; cnt++) {
      swalk_RandomWalk1(gen, res4, 1, size, r, 30, 150, 150);
      GetPVal_Walk(1, res4, &j, "", j2);
      
      // 5 values to check
      if(!isQuestionablePvalue(bbattery_pVal[pv_offset  ]) &&
	 !isQuestionablePvalue(bbattery_pVal[pv_offset+1]) &&
	 !isQuestionablePvalue(bbattery_pVal[pv_offset+2]) &&
	 !isQuestionablePvalue(bbattery_pVal[pv_offset+3]) &&
	 !isQuestionablePvalue(bbattery_pVal[pv_offset+4]))
	break;
      j = pv_offset;
      printf(" : ? RandomWalk\n");
      size += size >> 2;
...
void bbattery_SmallCrushAdaptive(unif01_Gen * gen)
{
  ...
  for(i = 1; i <= SMALLCRUSH_NUM; ++i)
    Rep[i] = 1;
  SmallCrushAdaptive(gen, NULL, Rep);
}
